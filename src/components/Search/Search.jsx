...
const Search = ({
	...
	const handleSearch = async () => {
		...
		try {
			...
			toast.error(translations.bad_address_or_herotag, {
				theme: "dark",
		...
		<>
			{!!redirect ? (
				...
				<div className="d-flex flex-column flex-grow-1 my-5">
					...
					<div
						className="d-flex flex-column flex-lg-row my-5"
						style={{ gap: "1em" }}
					>
						<div className="form flex-grow-1">
							<div className="d-flex align-items-center form-group">
								<MultiversxIcon
									style={{
										margin: "0 0.6em",
										width: `${elrondLogoFontSize * 1.6}px`,
										height: `${elrondLogoFontSize * 1.6}px`,
										fill: "white",
								...
								<input
									...
									type="text"
									name="egld address"
									className="form-style maiar-shadow text-truncate"
									placeholder={
										translations?.write_here_your_egld_address_or_herotag
									}
									autoComplete="off"
								/>
							</div>
						...
						<button
							...
							className="btn d-none d-lg-flex maiar-shadow"
						>
							<FaSearch size={24} />
						</button>
						<button
							...
							disabled={!value || loading}
							style={{ height: `${inputHeight}px` }}
							className="btn d-lg-none maiar-shadow"
						>
							{translations?.track_my_rewards}
						</button>
					</div>
					{searchHistory.length > 0 && (
						<div className="search-history d-flex flex-column justify-content-center">
								...
								{searchHistory.map((search, index) => {
									return (
										<div
											key={search}
											className="d-grid"
											style={{ gridTemplateColumns: "9fr 1fr" }}
										>
											<NavLink
												to={`/rewards/${search}`}
												key={index}
											...
											{index === searchHistory.length - 1 ? (
												<CardFooter className="d-flex justify-content-center align-items-center">
													<Button
														onClick={() => removeFromSearchHistory(search)}
														className="p-2 remove"
													>
														<AiFillCloseCircle size={LOGO_FONT_SIZE} />
													</Button>
												</CardFooter>
											) : (
												<CardHeader className="d-flex justify-content-center align-items-center">
...
const mapStateToProps = ({ i18n, account, searchHistory }) => ({
...
const mapDispatchToProps = dispatch => ({
	...
	removeFromSearchHistory: search => {
		dispatch(removeFromSearchHistory(search));
	}
});
